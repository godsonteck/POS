...
export interface CheckoutInput {
  saleId: string; receiptNumber: string; staffId: string; shiftId?: string;
  lines: CheckoutLine[]; paymentMethod: PaymentMethod; amountTendered?: Money;
  momoReference?: string; nowIso: string;
}
...
export function completeCheckout(db: Database.Database, input: CheckoutInput): CheckoutResult {
  ...
  db.transaction(() => {
    ...
    for (const line of input.lines) {
      ...
      db.prepare('UPDATE products SET quantity_in_stock = quantity_in_stock - ?, updated_at = ? WHERE id = ?').run(line.quantity, input.nowIso, line.productId);
